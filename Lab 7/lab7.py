...
def XOR(x,y):
    '''Returns a true output when the number of inputs is odd. Otherwise returns false.''' 
...
def fourBitAdd(xxxx,yyyy):
    ...
    and c is the carry.'''
    (c, xe) = FA(xxxx[3], yyyy[3], 0)
    (d, xf) = FA(xxxx[2], yyyy[2], c)
    (e, xt) = FA(xxxx[1], yyyy[1], d)
    (f, xo) = FA(xxxx[0], yyyy[0], e)
    return (f, (xo, xt, xf, xe))
