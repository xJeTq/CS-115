...
def testInsert1(ins):
    ...
    print("InsertV1 - Correct") 

testInsert1(insertV1)
...
def testInsert2(ins):
    '''Assume ins is a function. Test whether it solves the insert problem.
    For example, testInsert(insertV2).'''
    ...
    print("InsertV2 - Correct") 

testInsert2(insertV2)
...
def letterCounts(fname):
    ...
    for x in f:
        for y in x:
            if letter(y) in letterDict:
                letterDict[letter(y)] += 1
            else:
                letterDict[letter(y)] = 1
    L = []
    for z in letterDict.keys():
        if z != 0:
            L.append((letterDict[z], z))
    L.sort()
    L.reverse()
...
letterCounts("small_file.txt") # Correct 
